...
With the constant growth of the electrical system, the efficiency in the maintence execution methods at these enviroments is becoming increasingly important. The tomographic inspection is among alternatives since its efficient on foreseeing equipment fail on energy substations. However,  the operators safety and the need for an high expertise level makes the activity overly expensive. The key idea presented on this work is to propose an partial autmation on the tomographic inspection process of the electrical energy substations, using YOLOv3 and the ensemble random forest for the equipment detection on energy substations. Using a disconect switches image database from a electrical substation, the technology can identify and segment heating regions in a satisfying way and thus help energy concessionaires to make decisions on predictive maintence. The neural network achieved precision levels above 70%, wich shows off an satisfiying performance.
## <div>Introdução</div> 
